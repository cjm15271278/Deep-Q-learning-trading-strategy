...
class DeepQNetwork:
    def __init__(
            ...
            output_graph=False,
            #model_path = "/content/drive/My Drive/DQN/Best_Model/"
    ):
    ...
    def learn(self): # 如何学习，更新参数的，设计target_net和eval_net的交互使用
        ...
        #判断cost是否为最小cost，如果是就保存，反之不保存
        try:
            if self.cost<min(self.cost_his):
                print("此时保存模型，此时最小cost为："+str(self.cost))
                saver = tf.train.Saver()
                self.sess.run(tf.global_variables_initializer())
                #saver.save(self.sess, "Best_Model/model" + ".ckpt")
