...
class stock:
    ...
    def __init__(self, trend, init_money=10000, window_size=400): #这里把df改为trend
        
        self.n_actions = 3 # 动作数量
        self.n_features = window_size  #+  iisignature.siglength(2,1) # 特征数量   
        #self.trend = df['close'].values # 收盘数据
        #self.df = df #数据的DataFrame
        self.trend = trend # 收盘数据
        self.init_money = init_money # 初始化资金
        self.n_lagged_time_steps = 200  #用于计算signature的p-lag value
    ...
    def get_info(self):
        return self.states_sell, self.states_buy, self.profit_rate_account, self.profit_rate_stock, self.daily_return 
    #获得信息函数：卖出天数、买入天数、账户利率、股票利率
    def draw(self,name):
        # 绘制结果
        states_sell, states_buy, profit_rate_account, profit_rate_stock, daily_return = self.get_info()
