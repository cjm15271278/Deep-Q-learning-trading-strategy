...
class stock:
    ...
    def __init__(self, trend, init_money=10000, window_size=6): #这里把df改为trend
        ...
        self.n_lagged_time_steps = 6  #用于计算signature的p-lag value
    ...
    def signature_matrix(self, market_info ): # compute the signature matrix
        ...
        for i in range(X_ts.shape[0]):
            ...
            sig = iisignature.sig(path,1)   # change the dimension of signature
            sig = sig.tolist()
            sig_m.append(sig)
        return sig_m
    ...
    def get_state(self, t): #某t时刻的状态
        ...
        #signature = iisignature.sig(path,1)     # iisignature的结果为np.array
